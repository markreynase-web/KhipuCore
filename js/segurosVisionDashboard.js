// js/segurosVisionDashboard.js
// Dashboard bespoke de Seguros de Visión: mismo patrón que
// segurosDentalesDashboard.js (flujo de estados real vía PUT).
...
export function renderSegurosVisionDashboard(filasCrudas, backend) {
...
function asegurarEventos() {
  ...
  eventosListos = true;
  document.getElementById('segVisLista').addEventListener('click', (e) => {
...
function dibujarKpis() {
  ...
  document.getElementById('segVisKpis').innerHTML = [
...
function dibujarLista() {
  ...
  document.getElementById('segVisCount').textContent = `${ordenados.length} reclamo(s)`;

  document.getElementById('segVisLista').innerHTML = ordenados.length ? ordenados.map(r => {
    const est = ETIQUETA_ESTADO[r.estado] || { texto: r.estado, color: 'muted' };
    const sub = [r.aseguradora, r.orden_descripcion, escapeHtml(String(r.fecha || '').slice(0, 10))].filter(Boolean).join(' · ');
